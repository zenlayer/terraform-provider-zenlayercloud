...
* `bind_type` - (Optional, String, ForceNew) Elastic IP bind type. Valid values: `FullNat`, `Passthrough`. Effective when the elastic IP is assigned to a vNIC (`NIC`) or a HaVip (`HAVIP`).
